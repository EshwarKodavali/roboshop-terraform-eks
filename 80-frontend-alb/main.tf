...
resource "aws_route53_record" "ingress_alb" {
  zone_id = var.zone_id
  name    = "*.${var.domain_name}" # *.eshwar.fun
  type    = "A" # Or "AAAA" for IPv6
  allow_overwrite = true
  alias {
     # These are ALB details, not our domain details
    name                   = aws_lb.ingress_alb.dns_name # DNS name of the target resource
    zone_id                = aws_lb.ingress_alb.zone_id  # Hosted Zone ID of the target resource
